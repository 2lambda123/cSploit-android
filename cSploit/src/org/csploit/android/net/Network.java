...
public class Network implements Comparable<Network> {
  ...
  public Network(Context context) throws SocketException, UnknownHostException {
    ...
    if (isConnected() == false)
      throw new NoRouteToHostException(
          "Not connected to any WiFi access point.");
      ...
      try {
         ...
         *
         * It seems it's a ROM issue which doesn't correctly populate device
         * descriptors. This rom maps the default wifi interface to a generic
         * usb device ( maybe it's missing the specific interface driver ),
         * which is obviously not, and it all goes shit, use an alternative
         * method to obtain the interface object.
         */
        mInterface = NetworkInterface.getByName(
            java.lang.System.getProperty("wifi.interface", "wlan0"));
  ...
  public String getSSID() { return mWifiInfo.getSSID(); }

  public int getNumberOfAddresses() {
    return IP4Address.ntohl(~mNetmask.toInteger());
  }

  public IP4Address getStartAddress() { return mBase; }

  public String getNetworkMasked() {
    int network = mBase.toInteger();

    return (network & 0xFF) + "." + ((network >> 8) & 0xFF) + "." +
        ((network >> 16) & 0xFF) + "." + ((network >> 24) & 0xFF);
  }
  ...
  public DhcpInfo getInfo() { return mInfo; }

  public InetAddress getNetmaskAddress() { return mNetmask.toInetAddress(); }

  public InetAddress getGatewayAddress() { return mGateway.toInetAddress(); }
  ...
  public String getLocalAddressAsString() { return mLocal.toString(); }

  public InetAddress getLocalAddress() { return mLocal.toInetAddress(); }

  public NetworkInterface getInterface() { return mInterface; }
